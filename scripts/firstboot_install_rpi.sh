...
#      src/edgecv and pulls onnxruntime, the CPU inference runtime; no RKNN/NPU
#      on RPi)
#   3. enable the GPIO UART (/dev/serial0) for the MAVLink link to the FC
...
# (no NPU — edgecv runs on the CPU via onnxruntime, installed in step 2).
...
if [ "$n_models" -gt 0 ]; then
    ...
    warn "no *.onnx found in $QG_DIR/models — the ONNX tracker can't load. Check the checkout."
fi
...
# incl. its own deps such as onnxruntime (the CPU inference runtime the ONNX
# tracker backend uses on the RPi 4B; there is no NPU here).
run_user "$PIP" install -e "$QG_DIR"

# ── 3. GPIO UART for the FC link (/dev/serial0) ───────────────────────────────
say "3/4 enable GPIO UART for /dev/serial0 (MAVLink to the FC)"
...
(expected) — check the checkout, or commit the nanotrack .onnx weights to
QuadGuide's models/ dir, then re-run. To run WITHOUT the NN tracker at all, set
tracker.import to a cv2 tracker (e.g. cv2:TrackerKCF) in configs/rpi4b.yaml.
