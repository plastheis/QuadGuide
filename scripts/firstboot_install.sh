...
#   2. clone/update QuadGuide into the login user's home, build the venv
#      (--system-site-packages so it sees apt's cv2/numpy), and install pip deps
#      (this also installs edgecv, the tracking library — it lives in-tree at
#      src/edgecv and is packaged together with quadguide)
#   3. compile + enable the OV9281 device-tree overlay
#   4. install the systemd service + WiFi AP (delegates to scripts/install_sbc.sh)
# It clearly PRINTS the one step that needs your hardware and cannot be automated:
...
n_models=0   # set in step 2; referenced in the summary
run_user() { sudo -u "$LOGIN_USER" -H "$@"; }
say "user=$LOGIN_USER home=$HOME_DIR  quadguide=$QG_DIR"

# ── 1. system packages ────────────────────────────────────────────────────────
say "1/4 apt system dependencies"
...
say "2/4 clone/update QuadGuide (as $LOGIN_USER), venv + python deps"
...
# The tracker's RKNN model blobs are committed directly in QuadGuide's models/ dir
# (vendored real files, not LFS pointers) — the clone above brought them along.
n_models=$(find "$QG_DIR/models" -name '*.rknn' 2>/dev/null | wc -l) || true
if [ "$n_models" -gt 0 ]; then
    echo "  RKNN models present: $n_models *.rknn under $QG_DIR/models"
else
    warn "no *.rknn found in $QG_DIR/models — the NPU tracker backend can't load. Check the checkout."
fi
...
# with ModuleNotFoundError before any worker starts. edgecv (the tracking library,
# src/edgecv/) is part of the same package tree and is installed along with it,
# incl. its own deps such as onnxruntime; RKNN runtime is below.
...
apt-get install -y python3-rknnlite2 2>/dev/null || \
    warn "python3-rknnlite2 not installed — needed for edgecv's NPU trackers (nanotrack/yolo)."

# ── 3. OV9281 overlay (+ driver check) ────────────────────────────────────────
say "3/4 OV9281 camera overlay"
KREL="$(uname -r)"
...
# ── 4. systemd service + WiFi AP ──────────────────────────────────────────────
say "4/4 systemd autostart + WiFi AP"
...
After reboot, one thing still needs YOUR hardware (cannot be automated from afar):
...
cat <<EOF
RKNN tracker models: $n_models *.rknn found under $QG_DIR/models (vendored in the
QuadGuide checkout above — see configs/rk3588.yaml's tracker.params.model_dir). If
that count is 0, the checkout is incomplete — re-run this script or re-clone.
