...
def ch_cfg() -> ChannelConfig:
    ...
    return ChannelConfig(
        ...
        flight_mode_index=0,
        roll_pitch_scale=60.0,   # madflight maxRollRate / maxPitchRate (rate mode)
        yaw_rate_scale=160.0,    # madflight maxYawRate
    )
...
def test_decode_attitude_imu_gyro_matches_att_rates():
    ...
    frame2 = CRSFFrame(
        type=CRSF_ATTITUDE,
        payload=struct.pack(">hhh", 0, 10000, 0),  # roll=1 rad (10000 * 1e-4)
...
def _decode_channels(frame_bytes: bytes) -> list[int]:
    payload = frame_bytes[3:25]  # skip sync(1), len(1), type(1); payload = 22 bytes
...
def test_encode_rc_none_center_roll_pitch_yaw(ch_cfg):
    ...
    assert channels[0] == 992   # ch1 roll neutral (1500µs)
...
def test_encode_rc_none_min_throttle(ch_cfg):
    channels = _decode_channels(encode_rc(None, armed=False, ch_cfg=ch_cfg))
    assert channels[2] == us_to_ticks(1100.0)   # ch3 throttle min


def test_encode_rc_none_disarmed(ch_cfg):
    channels = _decode_channels(encode_rc(None, armed=False, ch_cfg=ch_cfg))
    assert channels[4] == 172   # ch5 disarmed (988µs)


def test_encode_rc_armed_sets_ch5(ch_cfg):
    channels = _decode_channels(encode_rc(None, armed=True, ch_cfg=ch_cfg))
    assert channels[4] == 1811  # ch5 armed (2012µs)


def test_encode_rc_ch6_flight_mode_pos0(ch_cfg):
    channels = _decode_channels(encode_rc(None, armed=False, ch_cfg=ch_cfg))
    assert channels[5] == us_to_ticks(1165.0)   # ch6 flight mode position 0


def test_encode_rc_ch7_to_ch16_neutral(ch_cfg):
    channels = _decode_channels(encode_rc(None, armed=False, ch_cfg=ch_cfg))
    for i in range(6, 16):
        assert channels[i] == 992   # ch7–16 unused, neutral (1500µs)


def test_encode_rc_roll_full_right(ch_cfg):
    # roll_deg = +roll_pitch_scale (60 dps) → full stick → roll_max_us = 1900µs
...
def test_encode_rc_roll_full_left(ch_cfg):
    # roll_deg = -roll_pitch_scale (60 dps) → full stick → roll_min_us = 1100µs
    cmd = ControlCmd(timestamp_ns=0, roll_deg=-ch_cfg.roll_pitch_scale, pitch_deg=0.0,
                     yaw_rate_dps=0.0, throttle_norm=0.0)
    channels = _decode_channels(encode_rc(cmd, armed=False, ch_cfg=ch_cfg))
...
def test_encode_rc_throttle_half(ch_cfg):
    # 0.5 → 1100 + 0.5×800 = 1500µs → 992 ticks
...
def test_encode_rc_valid_crsf_frame(ch_cfg):
    ...
    assert data[0] == 0xC8              # sync byte
    assert data[2] == CRSF_RC_CHANNELS  # frame type
    assert len(data) == 26              # sync+len+type+22payload+crc
